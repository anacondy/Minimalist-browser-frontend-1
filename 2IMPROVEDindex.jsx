...
export default function App() {
  ...
  // Mock data for Tabs, inspired by the staggered schedule image
  // Included YouTube Music and Physics/Stats as per your preferences!
  const openTabs = [
  ...
  // Mock data for History, styled like the cast list
  const historyData = [
  ...
  // Function to handle executing a search when pressing Enter
  const handleSearchSubmit = (e) => {
    if (e.key === 'Enter' && searchQuery.trim() !== '') {
      // Open the search query in a new tab (using Google as default search engine)
      window.open(`https://www.google.com/search?q=${encodeURIComponent(searchQuery)}`, '_blank');
      // Clear the search bar after pressing enter
      setSearchQuery('');
      // Optionally reset view to main
      setActiveView('main');
    }
  };
  ...
  return (
    ...
    <div className="min-h-screen bg-black text-neutral-200 font-sans overflow-hidden relative selection:bg-white selection:text-black">
      ...
      {/* Injecting CSS globally to hide those ugly white default scrollbars entirely */}
      <style>
        {`
      ...
      {/* --- TABS VIEW --- */}
      {/* Added 'no-scrollbar' class to fix the white bar issue while scrolling */}
      ...
      {/* --- HISTORY VIEW --- */}
      {/* Added 'no-scrollbar' class here as well to fix the hover/scroll white bar bug */}
      <div className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${activeView === 'history' ? 'opacity-100 scale-100 z-20' : 'opacity-0 scale-105 pointer-events-none'}`}>
        <div className="no-scrollbar text-center space-y-2 max-h-[70vh] overflow-y-auto w-full px-4">
          {historyData.map((item, i) => (
            <div key={i} className="text-lg md:text-2xl tracking-wide hover:scale-105 transition-transform cursor-pointer py-1">
        ...
        <div className="w-full max-w-2xl relative group">
          ...
          <div className="relative flex items-center bg-black border-b-2 border-neutral-800 group-focus-within:border-white transition-colors duration-500 px-4 py-4 md:py-6">
            ...
            <input
              ...
              onKeyDown={handleSearchSubmit} // Added listener for Enter key presses
